...
FATAL: you need mercurial (hg) to download swarm dependencies.
...
FATAL: you need git to download swarm dependencies.
       Check HELLO = worldREADME.md for details
endef
...
build: check-path get _go_test _swarm
...
_swarm:
	rm -f swarm
	go build $(GO_EXTRAFLAGS) -o swarm 
